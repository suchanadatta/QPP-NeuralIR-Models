...
def writeOutFiles(train1, train2, train3, train4, test, foldnumber):
    ...
    with open('/store/causalIR/drmm/clueweb_exp/5-folds/'+str(qrel_name)+'_fold_'+str(foldnumber)+'.train', 'w') as trainFile:
    ...
    with open('/store/causalIR/drmm/data_qpp/5-folds/' + str(qrel_name) + '_fold_' + str(foldnumber) + '.test', 'w') as testFile:
        lines = []
        for topic in test:
...
if not os.path.exists('/store/causalIR/drmm/data_qpp/5-folds/'):
    os.makedirs('/store/causalIR/drmm/data_qpp/5-folds/')
