...
topic_ids = [id for id in qrels_doc_pairs]
# print("TOPIC IDs : ", qrels_doc_pairs)

# here we could shuffle things around

part_size = int(round(len(topic_ids)/4,0))
...
part_4 = topic_ids[3*part_size:]

print('part sizes 1-4:', len(part_1), len(part_2), len(part_3), len(part_4))
print('parts:\n[0] ',' '.join(part_1), '\n[1] ',' '.join(part_2), '\n[2] ',' '.join(part_3), '\n[3] ',' '.join(part_4))
...
# combine the parts
def writeOutFiles(train1, train2, train3, test, foldnumber):

    print('saving fold '+str(foldnumber))

    with open('/store/causalIR/drmm/clueweb_exp/4-folds/'+str(qrel_name)+'_fold_'+str(foldnumber)+'.train', 'w') as trainFile:
    ...
    with open('/store/causalIR/drmm/clueweb_exp/4-folds/' + str(qrel_name) + '_fold_' + str(foldnumber) + '.test', 'w') as testFile:
        lines = []
        for topic in test:
...
if not os.path.exists('/store/causalIR/drmm/clueweb_exp/4-folds/'):
    os.makedirs('/store/causalIR/drmm/clueweb_exp/4-folds/')

# train 1,2,3, test 4
writeOutFiles(part_1,part_2,part_3,part_4, 1)

# train 1,2,4 test 3
writeOutFiles(part_1,part_2,part_4,part_3, 2)

# train 1,3,4 test 2
writeOutFiles(part_1,part_3,part_4,part_2, 3)

# train 2,3,4 test 1
writeOutFiles(part_2,part_3,part_4,part_1, 4)
