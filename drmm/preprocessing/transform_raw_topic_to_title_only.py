...
outFilepath = '/store/causalIR/drmm/data/'+os.path.basename(sys.argv[1])
if doStem:
    outFilepath = '/store/causalIR/drmm/data/' + os.path.splitext(os.path.basename(sys.argv[1]))[0] +".stemmed"\
...
with open(outFilepath, 'w') as outputFile:
    with open(sys.argv[1], 'r') as inputFile:
        ...
        for inLine in inputFile.readlines():
            ## fix this (some extra text comes up)
            if inLine.startswith('<num>'):
                currentId = inLine.replace('<num>', '').strip()
